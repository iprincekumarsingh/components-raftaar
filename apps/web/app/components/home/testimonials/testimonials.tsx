...
const Testimonials = ({ ...testimonialsData }) => {
  const { testimonials, title, subHeading } = testimonialsData;

  // Concatenate testimonials array with itself for seamless looping
  const concatenatedTestimonials = [...testimonials, ...testimonials];

  const halfIndex = Math.ceil(concatenatedTestimonials.length / 2);
  const firstHalf = concatenatedTestimonials.slice(0, halfIndex);
  const secondHalf = concatenatedTestimonials.slice(halfIndex);
    ...
    <div className="md:h-[700.78px] md:overflow-hidden md:py-20">
      ...
      >
          ...
          <div className="justify-center items-start inline-flex">
            <div className="grow shrink basis-0 self-stretch md:pb-[72.78px] flex-col justify-start items-center gap-6 inline-flex">
              <div className="flex-col justify-start items-start gap-4 flex">
                <div className="w-72 pt-[11px] mt-20 justify-center items-start inline-flex">
        ...
        <div className="md:hidden flex-col justify-start items-start flex gap-4 max-hd ">
          {concatenatedTestimonials.map((testimonial, index) => (
...
const Card = ({ name, designation, image, testimonial }) => {
  return (
